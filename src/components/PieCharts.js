...
class PieCharts extends Component {
	render() {
		...
		if (tickerData && globalData) {
			...
			var coinne = tickerArray[0];
			var coinwo = tickerArray[1];
			var coinhree = tickerArray[2];

			// var pieChartData = tickerArray.map( coin=> coinname, coinquotes.USD.market_cap )
			var pieChartDataTopOneHundred = tickerArray.map(function (coin, {
				return [coinname, coinquotes.USD.market_cap];
			});
			...
			var pieChartDataTotalMarketCap = [
				...
				[
					'Rest of Total Crypto Market Cap',
					globalData.quotes.USD.total_market_cap -
						tickerArray[0].quotes.USD.market_cap -
						tickerArray[1].quotes.USD.market_cap -
						tickerArray[2].quotes.USD.market_cap,
				],
			];
          ...
          This is the PieCharts component.
				...
				<div id="charts">
					<div id="pie1" className="pie">
						<Segment raised>
							<h2>Top ten cryptocurrencies by market cap:</h2>
					...
					<div id="pie3" className="pie">
						<Segment raised>
							<h2>
								Top three cryptocurrencies by market cap, as a fraction of the total market cap of
							...
							<div>
								...
								{globalData ? globalData.active_cryptocurrencies.toLocaleString() : 'shrug'}
							</div>
							<div>
							...
							<div>
								...
								{globalData ? globalData.quotes.USD.total_volume_24h.toLocaleString() : 'shrug'}
							</div>

							<br></br>

							{/* this lists bitcoinpercentage from globalData, but i want top three coin, so using coindata... */}
							{/* <div>
                      Bitcoinpercentage of market cap: {globalData ? globalData.bitcoinpercentage_of_market_cap.toLocaleString() : "shrug"}%
                    </div> */}

							<div>
								The #1 ed coin {tickerData && globalData ? coinne.name : 'no data'}, percentage of
								the total market cap:{' '}
								{tickerData && globalData
									? (
											(coinne.quotes.USD.market_cap / globalData.quotes.USD.total_market_cap) *
											100
									  ).toFixed(2)
									: 'no data'}
								%
							...
							<div>
								The #2 ed coin {tickerData && globalData ? coinwo.name : 'no data'}, percentage of
								the total market cap:{' '}
								{tickerData && globalData
									? (
											(coinwo.quotes.USD.market_cap / globalData.quotes.USD.total_market_cap) *
											100
									  ).toFixed(2)
									: 'no data'}
								%
							...
							<div>
								The #3 ed coin {tickerData && globalData ? coinhree.name : 'no data'}, percentage
								of the total market cap:{' '}
								{tickerData && globalData
									? (
											(coinhree.quotes.USD.market_cap / globalData.quotes.USD.total_market_cap) *
											100
									  ).toFixed(2)
									: 'no data'}
								%
							...
							<div>
								Percentage of the total market cap, minus the top three ed coin:{' '}
								{globalData && tickerData
									? (
											((globalData.quotes.USD.total_market_cap -
												coinne.quotes.USD.market_cap -
												coinwo.quotes.USD.market_cap -
												coinhree.quotes.USD.market_cap) /
												globalData.quotes.USD.total_market_cap) *
											100
									  ).toFixed(2)
									: 'no data'}
							...
							<div>
								Total cryptocurrency market cap (USD): $
								{globalData ? globalData.quotes.USD.total_market_cap.toLocaleString() : 'shrug'}
							</div>
